...
<script lang="ts">
  ...
  // custom simulation force - rectangular rather than circle with bounds
  ...
  // core d3 code
  function drawChart() {
    // define svg props + chart dimensions
    ...
    svg
      .select('#fade-gradient-stop1')
      .attr('offset', '0%')
      .attr('stop-color', 'white')
      .attr('stop-opacity', 0);

    svg
      .select('#fade-gradient-stop2')
      .attr('offset', '100%')
      .attr('stop-color', 'white')
      .attr('stop-opacity', 1);

    svg
      .select('#rightFadeMaskRect')
      .attr('width', bookWidth)
      .attr('height', bookHeight)
      .attr('fill', 'white');

    // arrow rects + lines - declared individual as custom and easier to read
    // lines are rects to allow for the gradients
    svg
      .select('#rectLeft')
      .attr('x', margin.left)
    ...
    // quadrant groups - rect, label, marker
    const quadrantGroup = svg
      .select('#quadrantsGroup')
    ...
    // set bounds for force simulation
    const bounds = {
      x0: 0,
      y0: 0,
    ...
    // nodes group - mask allows for fading out
    ...
    // separate outline so it's visible over the mask
    nodesGroup
      .select('.outlineRect')
      .attr('rx', 3)
      .attr('ry', 3)
    ...
    // author + title labels shifted up and down to align centrally
    nodesGroup
      .select('.authorLabel')
      .attr('pointer-events', 'none')
      .attr('x', 5 + bookHeight)
    ...
    // simulation ticks for node transform
    simulation.on('tick', () => {
      nodesGroup.attr(
        'transform',
        (d) => `translate(${margin.left + d.x},${margin.top + d.y})`,
